...
async function spawnWorldBoss(channels) {
  ...
  const payloadMsg = {
    content: `@here 🚨 **ALERTA GLOBAL: UM WORLD BOSS APARECEU!** 🚨\n\nUma aberração colossal de **${bossConfig.hp} HP** invadiu o chat! Vocês têm **${Math.round(bossConfig.expireMs / 60000)} minutos** para derrotá-lo!\nQuem participar da batalha dividirá o grande prêmio de **${bossConfig.prize} Nanacoins 🪙** com base no dano causado.\n\nHP do Boss: \`${bossConfig.hp} / ${bossConfig.hp}\``,
    components: [row]
  };
...
async function handleBossInteraction(interaction) {
  ...
  // Define cooldown
  bossInstance.cooldowns.set(userId, now + bossConfig.attackCooldownMs);

  const dano = Math.floor(Math.random() * (bossConfig.damageMax - bossConfig.damageMin + 1)) + bossConfig.damageMin;
  ...
  if (bossInstance.hp > 0) {
    ...
    interaction.message.edit({
      content: interaction.message.content.replace(/HP do Boss: `\d+ \/ \d+`/, `HP do Boss: \`${bossInstance.hp} / ${bossInstance.maxHp}\``)
    }).catch(() => null);
    ...
    const prize = bossConfig.prize;
    const totalBossDamage = bossInstance.maxHp;
